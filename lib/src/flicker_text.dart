...
class FlickerText extends StatefulWidget {
  ...
  final bool tapShow; // New parameter to enable/disable tap-to-show functionality
  final int showDurationInSeconds; // New parameter to set the duration in seconds for showing the text
...
class _FlickerTextState extends State<FlickerText> with TickerProviderStateMixin {
  ...
  late final TapGestureRecognizer _onTapRecognizer = TapGestureRecognizer()
    ..onTapUp = (details) {
      ...
      if (widget.tapShow && spoilerRects.any((rect) => rect.contains(fadeOffset))) {
        _showTextTemporarily();
      } else if (widget.enable && spoilerRects.any((rect) => rect.contains(fadeOffset))) {
        setState(() {
